...
def handler_md_late_night(match, base_year):
    ...
    return safe_datetime_with_25h(base_year, month, day, hour, minute)
...
patterns_with_handlers = [
    # 1. 年あり／なしの月日＋時刻（全角半角混在、曜日あり／なし、スラッシュ、漢数字時分対応）
    # {
    #     "id": 1,
    #     "pattern": re.compile(
    #         r"""(?:(?P<year>\d{4})[年/])?                          # 年(任意)
    #             (?P<month>[０-９\d]{1,2}|[一二三四五六七八九十]{1,3})月|/   # 月
    #             (?P<day>[０-９\d]{1,2}|[一二三四五六七八九十]{1,3})日?    # 日
    #             (?:（?[日月火水木金土]\）?)?                        # 曜日(任意)
    #             \s*(?:より|から)?                                 # 接続語
    #             (?:毎週[^\s　]*\s*)?                              # 毎週〜(任意)
    #             (?:(?P<late_night>深夜)?(?P<hour>[０-９\d]{1,2})  # 時刻(時)
    #                (?:[:：時](?P<minute>[０-９\d]{2})分?)?)       # 分
    #         """, re.VERBOSE
    #     ),
    ...
    # # 2. 年あり／なしの月日のみ（時刻なしは 00:00 扱い）
    # {
    #     "id": 2,
    #     "pattern": re.compile(
    #         r"""(?:(?P<year>\d{4})[年/])?
    #             (?P<month>[０-９\d]{1,2}|[一二三四五六七八九十]{1,3})月|/
    #             (?P<day>[０-９\d]{1,2}|[一二三四五六七八九十]{1,3})日?
    #             (?:（?[日月火水木金土]\）?)?
    #         """, re.VERBOSE
    #     ),
    ...
    #     "confidence": 4,
    # },    
    # 3. 曜日＋時刻のみ（「毎週火曜 24:30」など）
    {
        "id": 3,
        "pattern": re.compile(
            r"""毎週(?:月|火|水|木|金|土|日)曜
                (?:深夜)?\s*
                (?P<hour>\d{1,2})[:：時](?P<minute>\d{2})分?
            """, re.VERBOSE
        ),
    ...
    # 4. 時刻のみ（24時以上対応、翌日扱い）
    {
        "id": 4,
        "pattern": re.compile(
            r"""(?P<hour>\d{1,2})[:：時](?P<minute>\d{2})分?
                \s*(?:配信|放送|開始)?
            """, re.VERBOSE
        ),
